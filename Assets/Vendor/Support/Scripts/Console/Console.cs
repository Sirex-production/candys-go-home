...
namespace Support.Console
{
    ...
    public sealed class Console : MonoSingleton<Console>
    {
        ...
        [Inject] private readonly PcInputService _pcInputSystem; 

        private readonly (Vector2 position, Vector2 size) INPUT_AREA_SCREEN_PROPERTIES = (new Vector2(0, 0), new Vector2(Screen.width, 50));
        private readonly (Vector2 position, Vector2 size) OUTPUT_AREA_SCREEN_PROPERTIES = (new Vector2(0, 50), new Vector2(Screen.width, 500));

        private LinkedList<IConsoleCommand> _consoleCommands = new();
        private string _history = "";
        private string _input = "";

        private bool _isActive = false;

        internal bool isThreadLogsEnabled = false;
        internal LinkedList<IConsoleCommand> CurrentConsoleCommands => new(_consoleCommands);
        ...
        protected override void Awake()
        {
            ...
            _pcInputSystem.OnConsoleInputReceived += OnConsoleInputReceived;
        }
        ...
        private void OnLogMessageReceivedThreaded(string message, string trace, LogType logType)
        {
            if(!isThreadLogsEnabled)
                return;

            _history += $"{logType}: {message}\n{trace}";
        }

        private void OnConsoleInputReceived()
        {
